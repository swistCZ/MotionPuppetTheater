...
* **Modular Architecture:**
  * `tracker.ts`: WebCam & MediaPipe Hands CV integration (Loads same-origin WASM files from `./mediapipe/` to pass DuckDuckGo / Brave privacy shields, with fallback to jsdelivr/unpkg CDNs).
...
* [x] Bundled MediaPipe WASM and solution data files locally in `public/mediapipe/` for **100% DuckDuckGo Privacy Browser & Brave compatibility** (bypasses third-party CDN tracker blocking)
