...
* [x] **Two-bone IK for elbows and knees**: rig arms/legs with an `elbow`/`knee` joint in their config are split into upper+lower sprites and posed by an analytic two-bone IK solver (`solveTwoBoneIK` in `rig.ts`, unit-tested for exact hand-on-target placement in every reach/bend-sign case). The hand/foot reaches toward the tracked limb direction scaled by its magnitude (fist = tucked bent elbow, open palm = extended), the upper rotation stays clamped to `maxArmDelta`, and the bend is capped so limbs never fold inside-out. Legs keep an A-frame splay + forward swing, now driven by the leg limb vectors. Manual drag, mouse-wheel fine rotation, pose snapshots and edit-handle placement (handles sit on the hands/feet) all honor the split limbs. The builder gained **Loket L/P** and **Koleno L/P** placement modes with on-preview markers and defaults computed on part load; configs without a joint keep the old single-sprite rigid rotation for backward compatibility. Demo character SVGs updated with visible elbows/knees. 39 unit tests + build green.
...
* **Rig arms bend only at the shoulder — no elbow/knee motion (RESOLVED):**
  * *Příznak:* Paže a nohy rig loutek rotovaly jako jeden tuhý kus kolem ramene/kyčle; chybělo ohnutí v lokti a koleni.
  ...
  * *Oprava:* `RigArmDef.elbow` / `RigLimbDef.knee` (volitelné, kód-validované: v obrázku a níž než pivot). `rigAssets.buildBoneLimb()` štěpí texturu v linii kloubu na upper+lower sprite (lower je child upperu, anchor v kloubu) a předpočítá IK geometrii (`len1/len2/phi1/phi2`). `renderer.poseRigLimb()` řeší analytické dvoukostní IK (`rig.solveTwoBoneIK`, bendSign paže +1 / nohy −1), clampuje horní rotaci na `maxArmDelta` a ohyb na ±2.2 rad. Builder má módy Loket/Koleno + auto-defaulty a export/import. Demo postava dostala klouby. Ověřeno headless: pěst → loket ohnutý (elbow > 20 px od přímky rameno-ruka), otevřená dlaň → rovná paže, drag/snapshot uchovávají rotace dolních segmentů; 39 testů + build zelené.
