...
* **Active Task:** Two-part limb IK (v2) restructuring complete — elbows/knees are now separate part files with their own pivots and an `attach` joint point, builder has Loket/Předloktí/Koleno/Holeň placement modes with auto-centering, legacy configs migrate automatically. Changes are uncommitted (previous v1 IK is on `origin/main` as `23203d9`). Waiting for user approval to commit + push. Next: user can re-cut the character art into the 10 separate part files and assemble via `/builder.html`.
* **Next Steps:**
