...
* [x] **Two-bone IK for elbows and knees (v2 – separate part files)**: each limb is now **two independent part images** (upper arm + forearm, thigh + shin), each with its own pivot, connected by an **`attach`** point (`RigLimbUpperDef.attach` = elbow/knee in the *upper* image where the lower part hooks in). This replaced the v1 single-image split at a code-defined joint: the joint is now placed exactly in the image editor and the lower part auto-centers on it, so elbows/knees land precisely on the art. Analytic two-bone IK (`solveTwoBoneIK` in `rig.ts`, unit-tested) poses both segments; `phi1`/`len1` come from `attach`−pivot, `phi2`/`len2` from the lower part's farthest opaque pixel (tie-broken to stay on the limb axis, `scanLimbEnd` unit-tested). Legs use `restAngle + π/2` as their rest direction. The builder gained **Loket L/P**, **Předloktí L/P**, **Koleno L/P**, **Holeň L/P** placement modes with on-preview markers; defaults auto-computed on load (attach = bottom-center of the upper opaque area, lower pivot = top-center); legacy configs with `elbow`/`knee` are migrated to `attach` on import. Lower parts are optional — missing them keeps a rigid single-sprite limb. Demo character split into 10 part SVGs. 45 unit tests + build green.

## 3. Active Task & Next Steps
* **Active Task:** Multi-puppet support + „Záběr" view frame committed to `main` (2026-08-21). The long-awaited two-bone IK v2 (separate part files, `attach` joint, builder Loket/Předloktí/Koleno/Holeň modes) was already on `main` as `f04feca`. This batch adds: static extra puppets + floating „Správa loutek" panel, export crop frame „Záběr", builder pivot-click fix, `preview.open: false` guard, and the no-browser-windows hard rule. Next: user re-cuts character art into separate parts and assembles via `/builder.html`, then optionally tunes `ROT_DAMP`/`spreadFactor` after camera playtesting.
* **Next Steps:**
  ...
  2. Optional: tune `ROT_DAMP` / `ROT_ALPHA` and `spreadFactor` range after real-camera playtesting.
  3. Optional: extend multi-puppet beyond stop-motion (live-slot switching, more z-order controls).
...
* **Rig arms bend only at the shoulder — no elbow/knee motion (RESOLVED, v2 restructured):**
  * *Příznak:* Paže a nohy rig loutek rotovaly jako jeden tuhý kus kolem ramene/kyčle; chybělo ohnutí v lokti a koleni. Navíc uživatel nemohl umístit loket/koleno přesně tam, kam chtěl (v1 ho určoval kód jako průsečík, ne obrázek).
  * *Skutečná příčina:* Model měl jediný kloub (pivot) na segment — textura končetiny se neštěpila a rotace šla celá na jeden sprite.
  * *Oprava:* Každá končetina = **dva samostatné part soubory s vlastními pivoty** (upper + lower) propojené bodem **`attach`** (loket/koleno v souřadnicích upper obrázku). `rigAssets.buildBoneLimb()` postaví upper sprite + lower sprite v child containeru na `attach − pivot` a předpočítá IK geometrii (`len1/len2/phi1/phi2`; `phi2`/`len2` z nejvzdálenějšího opákního pixelu lower dílu, tie-break na osu končetiny — `scanLimbEnd`). `renderer.poseRigLimb()` řeší analytické dvoukostní IK (`rig.solveTwoBoneIK`, bendSign paže +1 / nohy −1), clampuje horní rotaci na `maxArmDelta` a ohyb na ±2.2 rad. Builder má módy Loket/Předloktí/Koleno/Holeň s auto-defaulty (attach = bottom-center upper opák. oblasti, lower pivot = top-center), click-to-place v preview a migraci starých `elbow`/`knee` configů → `attach`. Ověřeno headless na pozadí: pěst → loket ohnutý, otevřená dlaň → rovná paže, kloub sedí přesně na attach bodě (phi = π/2), koleno se ohýbá, snapshots uchovávají rotace; 45 testů + build zelené.
...
* [ ] **Multi-puppet support (více než 2 figurky na scéně):**
  * **[x] Základní podpora (fáze 1, DONE 2026-08-21)**: Renderer umí libovolný počet **extra statických loutek** (`extraPuppets`) vedle živých L1/L2 — `addExtraPuppet`, `removeExtraPuppet`, `reorderExtraPuppets` (z-order), gettery. `StagePoseSnapshot` má nové volitelné pole `puppets[]` (zpětně kompatibilní, staré `.mpt` se načítají beze změny verze). Migrační helper `migrateStagePoseSnapshot` (src/snapshot.ts, unit-tested) normalizuje pózy při načtení projektu.
  * **[x] Plovoucí okno „Správa loutek" (DONE 2026-08-21)**: zobrazí se ve stop-motion + Ruka VYP. Draggable/dockable, sbalitelné na proužek. Nahoře **paleta postav** (Liška/Robot + rigy z builderu + uložené v prohlížeči) — klik = přidat na výchozí pozici, drag & drop na scénu (umístění na místo puštění). Dole **seznam vrstev** (z-order, klik = výběr, přetažení = přerovnání) + tlačítko **Smazat vybranou loutku**. Vybraná loutka je zvýrazněná **zlatým kroužkem kolem středu na scéně** (chycení kterékoli části loutky ji vybere).
  * **[x] Opravy zpětné vazby (DONE 2026-08-21)**: odstraněno matoucí tlačítko „×" z hlavičky; šipka minimalizace teď skutečně sbalí okno (přidán `stopPropagation` + práh pohybu, aby klik na šipku nespouštěl dockování); přidáno klikací přidávání do palety; smazání funguje přes tlačítko „Smazat vybranou loutku"; výběr loutky na scéně i v seznamu funguje a ukazuje zlatý kroužek.
  * **[x] Minimalizovaný proužek = lišta ovládání (DONE 2026-08-21)**: sbalené okno ukazuje kompaktní lištu s počtem loutek + tlačítky **Duplikovat** a **Smazat vybranou** (působí na vybranou loutku). Přidáno **Duplikovat** i do rozbaleného stavu.
